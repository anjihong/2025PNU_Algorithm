...
sort(table.begin(), table.end(), compare);
    ...
    {
        if (machine_table[table[i].day - 1].size() == k) not_use.push_back(table[i]);
        else machine_table[table[i].day - 1].push_back(table[i]);
    }
    ...
    for (int i = T - 1; i >= 0; i--)
    {
        for (int j = 0; j < k; j++)
        {
            ...
            {
                if (not_use[idx].day >= i + 1 && not_use[idx].pay > machine_table[i][j].pay) swap(not_use[idx], machine_table[i][j]);
